...
# ── Aggregation: raw_bird_observation → aggregated_bird_observation ────────────

def aggregate_raw_to_agg(years: list, conn) -> None:
    """
    For any year in `years` not yet present in aggregated_bird_observation,
    aggregate raw_bird_observation (joined with species_masterlist) and insert
    the results. Years already in aggregated_bird_observation are skipped.
    """
    placeholders = ','.join(['%s'] * len(years))
    cur = conn.cursor()
    cur.execute(
        f"SELECT DISTINCT year FROM aggregated_bird_observation WHERE year IN ({placeholders})",
        years,
    )
    already_agg = {row[0] for row in cur.fetchall()}
    cur.close()

    years_to_agg = sorted(y for y in years if y not in already_agg)
    if not years_to_agg:
        emit('  aggregated_bird_observation already up to date for all target years.')
        return

    emit(f'  Aggregating raw observations for year(s): {years_to_agg}...')
    year_filter = ','.join(str(y) for y in years_to_agg)

    df_raw = load_sql(conn,
        f"""SELECT r.grid_lat, r.grid_lon, r.latitude, r.longitude,
                   r.month, r.year, r.bird_count, r.site_name,
                   r.species_id,
                   sm.species_name,
                   sm.light_tolerance, sm.migratory_status
              FROM raw_bird_observation r
              LEFT JOIN species_masterlist sm ON sm.species_id = r.species_id
             WHERE r.year IN ({year_filter})
               AND r.grid_lat IS NOT NULL AND r.grid_lon IS NOT NULL"""
    )

    if df_raw.empty:
        emit('  No raw observations found for target years — skipping aggregation.')
        return

    emit(f'  Loaded {len(df_raw):,} raw observation rows.')

    grp = ['grid_lat', 'grid_lon', 'month', 'year']

    num_agg = df_raw.groupby(grp).agg(
        bird_count=('bird_count', 'sum'),
        unique_species_count=('species_id', 'nunique'),
        latitude=('latitude', 'first'),
        longitude=('longitude', 'first'),
        site_name=('site_name', 'first'),
    ).reset_index()

    sp_uniq = df_raw.drop_duplicates(subset=grp + ['species_id'])

    def _count_where(mask_col, val):
        return (sp_uniq[sp_uniq[mask_col] == val]
                .groupby(grp).size().reset_index(name=f'_{mask_col}_{val}'))

    tol = _count_where('light_tolerance',  'tolerant')
    sen = _count_where('light_tolerance',  'sensitive')
    res = _count_where('migratory_status', 'resident')
    mig = _count_where('migratory_status', 'migratory')

    def _make_species_json(names):
        valid = sorted({n for n in names if n and not (isinstance(n, float) and pd.isna(n))})
        return _json.dumps(valid)

    sp_list = (df_raw.groupby(grp)['species_name']
               .apply(_make_species_json)
               .reset_index(name='species_list'))

    agg = num_agg
    for df_c, src_col, dst_col in [
        (tol, f'_light_tolerance_tolerant',   'total_tolerant'),
        (sen, f'_light_tolerance_sensitive',   'total_sensitive'),
        (res, f'_migratory_status_resident',   'total_resident'),
        (mig, f'_migratory_status_migratory',  'total_migratory'),
    ]:
        agg = agg.merge(df_c, on=grp, how='left')
        agg.rename(columns={src_col: dst_col}, inplace=True)

    agg = agg.merge(sp_list, on=grp, how='left')

    for col in ['total_tolerant', 'total_sensitive', 'total_resident', 'total_migratory']:
    ...
    for _, r in agg.iterrows():
        rows.append((
            str(r['site_name']) if r['site_name'] and not (isinstance(r['site_name'], float) and pd.isna(r['site_name'])) else None,
            float(r['latitude']),
            float(r['longitude']),
            int(r['month']),
            int(r['year']),
            int(r['total_resident']),
            int(r['total_migratory']),
            int(r['total_tolerant']),
            int(r['total_sensitive']),
            int(r['bird_count']) if not pd.isna(r['bird_count']) else 0,
            int(r['unique_species_count']),
            r['species_list'],
            float(r['grid_lat']),
            float(r['grid_lon']),
        ))
    ...
    for i in range(0, total, BATCH_SIZE):
        ...
        cur2.executemany(INSERT_AGG, chunk)
        conn.commit()
        inserted += len(chunk)
        emit(f'  Aggregation insert: {inserted:,}/{total:,} rows')
    cur2.close()
    emit(f'  Aggregation complete — {total:,} rows written to aggregated_bird_observation.')
...
def build(years: list) -> None:
    ...
    # ── Step 0: Populate aggregated_bird_observation for missing years ─────────
    conn0 = get_conn()
    emit('Step 0: Checking aggregated_bird_observation...')
    aggregate_raw_to_agg(years, conn0)
    conn0.close()
