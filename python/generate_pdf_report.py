...
The PDF mirrors the reports.php ecological and diagnostics sections,
including charts and the numerical values behind each chart.
"""
...
def write_csv(payload: Dict[str, Any], output_path: str) -> None:
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as f:
        ...
        w.writerow(["Area", filters.get("selected_area", "All Areas")])
        w.writerow(["Trend Range", f"{filters.get('start_year', '')} - {filters.get('end_year', '')}"])
        w.writerow(["Snapshot", f"{filters.get('snapshot_year', '')}-{str(filters.get('snapshot_month', '')).zfill(2)}"])

        section(w, "ECOLOGICAL IMPACT - HISTORICAL TRENDS")
        ...
        section(w, "ECOLOGICAL IMPACT - MIGRATION STATUS DISTRIBUTION")
        ...
        section(w, "ECOLOGICAL IMPACT - LIGHT TOLERANCE DISTRIBUTION")
        w.writerow(["Category", "Count"])
        ...
        scatter_defs = [
            ("light_richness", "ECOLOGICAL IMPACT - ALAN VS RICHNESS"),
            ("ndvi_richness", "ECOLOGICAL IMPACT - NDVI VS RICHNESS"),
            ("lst_richness", "ECOLOGICAL IMPACT - LST VS RICHNESS"),
            ("precipitation_richness", "ECOLOGICAL IMPACT - PRECIPITATION VS RICHNESS"),
        ]
        for key, title in scatter_defs:
        ...
        section(w, "ECOLOGICAL IMPACT - TOP SITES BY RICHNESS")
...
def make_historical_chart(payload: Dict[str, Any], tmp_dir: str) -> str:
    ...
    fig.suptitle("Historical Trends")
...
def place_chart(
    ...
    *,
    ...
    if not path or not os.path.isfile(path):
        return
    # Reserve note + image + trailing gap to keep following content from overlapping.
...
def write_pdf(payload: Dict[str, Any], output_path: str) -> None:
    ...
    summary_rows = [
        ["Area", filters.get("selected_area", "All Areas")],
        ["Trend Years", f"{filters.get('start_year', '-')}-{filters.get('end_year', '-') }"],
        ["Snapshot", f"{filters.get('snapshot_year', '-')}-{str(filters.get('snapshot_month', '-')).zfill(2)}"],
        ["Generated", generated],
    ]
    ...
    with tempfile.TemporaryDirectory(prefix="avilight_pdf_") as tmp_dir:
        ...
        mig_rows = [[mig_labels[i] if i < len(mig_labels) else "", fmt_num(mig_data[i] if i < len(mig_data) else 0, 0)] for i in range(max(len(mig_labels), len(mig_data)))]
        write_table_block(pdf, "Numerical Values: Migration Distribution", ["Category", "Count"], [130, 60], mig_rows, numeric_cols=(1,))
        lt_rows = [[lt_labels[i] if i < len(lt_labels) else "", fmt_num(lt_data[i] if i < len(lt_data) else 0, 0)] for i in range(max(len(lt_labels), len(lt_data)))]
        write_table_block(pdf, "Numerical Values: Light Tolerance Distribution", ["Category", "Count"], [130, 60], lt_rows, numeric_cols=(1,))

        scatter_specs: List[Tuple[str, str, str, str]] = [
            ("light_richness", "ALAN vs Richness", "ALAN (nW/cm2/sr)", "#dc2626"),
            ("ndvi_richness", "NDVI vs Richness", "NDVI (index)", "#16a34a"),
            ("lst_richness", "LST vs Richness", "LST (degC)", "#d97706"),
            ("precipitation_richness", "Precipitation vs Richness", "Precipitation (mm)", "#0891b2"),
        ]
        for key, title, xlabel, color in scatter_specs:
        ...
        for f in filters_order:
            ...
            imp_rows = [[labels[i] if i < len(labels) else "", fmt_num(values[i] if i < len(values) else None, 6)] for i in range(n_imp)]
            write_table_block(
                pdf,
                "Numerical Values: " + title,
                ["Feature", "Importance"],
                [130, 60],
                imp_rows,
                numeric_cols=(1,),
