...
$base = $pdo->prepare("SELECT COUNT(*) AS c FROM raw_bird_observation r JOIN observation_city_map m ON m.rbo_id=r.id WHERE r.year=:y AND r.month=:m AND r.species_id IS NOT NULL");
$base->execute([':y'=>$year, ':m'=>$month]);
$baseCount = (int)$base->fetch(PDO::FETCH_ASSOC)['c'];

$withEnv = $pdo->prepare("SELECT COUNT(*) AS c FROM raw_bird_observation r JOIN observation_city_map m ON m.rbo_id=r.id LEFT JOIN viirs v ON v.year=r.year AND v.month=r.month AND ABS(v.latitude-r.latitude)<0.000001 AND ABS(v.longitude-r.longitude)<0.000001 LEFT JOIN ndvi n ON n.year=r.year AND n.month=r.month AND ABS(n.latitude-r.latitude)<0.000001 AND ABS(n.longitude-r.longitude)<0.000001 LEFT JOIN land_temp lt ON lt.year=r.year AND lt.month=r.month AND ABS(lt.latitude-r.latitude)<0.000001 AND ABS(lt.longitude-r.longitude)<0.000001 LEFT JOIN precip p ON p.year=r.year AND p.month=r.month AND ABS(p.latitude-r.latitude)<0.000001 AND ABS(p.longitude-r.longitude)<0.000001 WHERE r.year=:y AND r.month=:m AND r.species_id IS NOT NULL AND (v.viirs_avg_rad IS NOT NULL OR n.ndvi IS NOT NULL OR lt.lst_day IS NOT NULL OR p.precip_mm IS NOT NULL)");
